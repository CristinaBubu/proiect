...
//DE AICI IN JOS SUNT DOAR MODALELE REATASATE DIN RATING_MODAL SI RECIPE_MODAL
//ALTFEL NU SE DESCHIDEAU IN FAVORITES,INDIFERENT CAT AM RASUCIT IMPORTURILE

// atentie la constructie de numire functie dincolo
import { modalRatingOpCl } from '../rating_modal-alina/rating.js';
...
function seeRecipe(evt) {
  ...
  if (evt.target.innerText === 'See recipe') {
    ...
    fetchRecipe(id).then(obj => {
      ...
      modalRatingOpCl(RatingBtn);
    });
  }
}

//ACEEASI SMECHERIE PT POPULAR RECIPES
...
function openModalRating() {   //atentie a se apela dincolo
  backdropEl.classList.add('is-hidden');
  document.body.classList.remove('no-scroll');
  document.removeEventListener('keydown', closoOnBackdrop);
}

//EXPORT PENTRU A O FOLOSI DINCOLO
...
//LE PUN AICI DIN CARDS-MARIAN CA SA NU NE MAI INCURCAM IN IMPORT
...
//EXPORT PT A O FOLOSI SI DINCOLO
